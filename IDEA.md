# LiteCode-TUI 完整详细技术方案 V1.3 最终定稿
## 文档说明
本文为 **LiteCode-TUI** 终端智能编程助手完整落地技术方案，整合全部需求：命名规范、UI视觉、交互逻辑、快捷键模式切换、Ollama连接配置、三级模型分工、本地工程能力、语法校验、UV工程支持、无密钥全网搜索、沙盒安全机制、内置代码资源库、Agent规划逻辑，所有规则、流程、功能、配置全部固化，可直接用于开发落地。
...
1. **顶部全局状态栏**
    展示内容：LiteCode标识、Ollama连接地址、当前三级模型名称、当前运行模式、工作目录路径、网络检索开关状态
2. **左侧面板**
    分区1：当前本地项目文件树
...
3. 严格沙盒限制：仅允许操作当前 litecode 启动目录及所有下级子目录
...
4. 本地缓存机制：高频技术资料存入`~/.litecode/cache/web_search`，重复需求优先读取本地缓存
...
# 本地代码参考库路径
code_base_path = "~/.litecode/code_base"
...
# 十、整体业务运行全流程
1. 终端进入目标项目文件夹，执行 `litecode` 启动程序
