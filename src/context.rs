    ...
    fn build_messages_truncates_old_history() {
        ...
        for i in 0..120 {
            ...
            history.push(msg(
                "user",
                &format!(
                    "message number {:03} with enough padding text to fill up",
                    i
                ),
            ));
            history.push(msg(
                "assistant",
                &format!(
                    "response {:03} with sufficient text padding content here now",
                    i
                ),
            ));
        }
        ...
        assert!(
            msgs.len() < history.len() + 2,
            "got {} msgs vs {} history+2",
            msgs.len(),
            history.len() + 2
        );
